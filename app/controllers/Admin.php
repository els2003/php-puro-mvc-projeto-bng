...
class Admin extends BaseController
{
    ...
    public function all_clients()
    {
        ...
        // get all clientes from all agents
    ...
    public function new_agent_submit()
    {
        ...
        if ($results['status'] == 'error') {
            ...
            // logger
            logger(get_active_user_name() . " - aconteceu um erro na criação de um novo registro de agente.");
            header('Location: index.php');
        }
        ...
        $results = $email->send_email(APP_NAME . ' Conclusão do registro de agente', 'email_body_new_agent', $data);
        if ($results['status'] == 'error') {

            // logger
            logger(get_active_user_name() . " - não foi possível enviar o email para conclusão de registro. " . $_POST['text_name']);
            die($results['message']);
        }

        // logger
        logger(get_active_user_name() . " - enviado com sucesso email para a conclusão de registro. " . $_POST['text_name']);
