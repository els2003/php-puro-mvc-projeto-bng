...
class Agent extends BaseController
{
    ...
    public function new_client_submit()
    {
        ...
        // check if the client already exits with the same name
        $model = new Agents();
        ...
        if($results['status']) {
            ...
            $_SESSION['server_error'] = 'Já existe um cliente com esse mesmo nome';
            $this->new_client_frm();
            return;
        }
    ...
    public function edit_client($id) 
    {
        ...
        // check if there are server error
        if (!empty($_SESSION['server_error'])) {
            $data['server_error'] = $_SESSION['server_error'];
            unset($_SESSION['server_error']);
        }
    ...
    public function delete_client_confirm($id) 
    {
        ...
        // loads the model to delete the clients data
        $model = new Agents();
        ...
        // returns to the agents main page
        $this->my_clients();
    }
    ...
    public function upload_file_submit() 
    {
        ...
        // check if there is a upload file
        ...
        // check if the upload file extension is valid
        $valid_extensions = ['xlsx', 'csv'];
        $tmp = explode('.', $_FILES['clients_file']['name']);
        $extension = end($tmp);
        if(!in_array($extension, $valid_extensions)){
            // logger 
            logger(get_active_user_name() . " - tentou carregar um arquivo inválido: " . $_FILES['clients_file']['name'], "error");
            
            $_SESSION['server_error'] = 'O arquivo deve ser do tipo XLSX ou CSV';
            $this->upload_file_frm();
            return;
        }
        ...
        if($_FILES['clients_file']['size'] > 2000000) {
            // logger 
            logger(get_active_user_name() . " - tentou carregar um arquivo inválido: " . $_FILES['clients_file']['name'] . " tamanho máximo excedido", "error");
        ...
        if(move_uploaded_file($_FILES['clients_file']['tmp_name'], $file_path)) {
            ...
            // validate the header
            $results = $this->had_valid_header($file_path);
            if ($results) {
                ...
                // logger 
                logger(get_active_user_name() . " - tentou carregar um arquivo com header incorreto: " . $_FILES['clients_file']['name'], "error");

                $_SESSION['server_error'] = "O arquivo não tem o header no formato correto.";
                $this->upload_file_frm();
                return;
            }
            ...
            // logger 
            logger(get_active_user_name() . " - aconteceu um erro inesperado no carregamento do arquivo: " . $_FILES['clients_file']['name'], "error");

            $_SESSION['server_error'] = 'Aconteceu um erro inesperado no carregamento do arquivo.';
            $this->upload_file_frm();
            return;
        }
    }
    ...
    // ===============================================================
    private function had_valid_header($file_path) 
    {
        // validates the file
        $data = [];
    ...
    private function load_file_data_to_database($file_path)
    {
        ...
        // logger 
        logger(get_active_user_name() . " - carregamento de arquivo efetuado: " . $_FILES['clients_file']['name']);
